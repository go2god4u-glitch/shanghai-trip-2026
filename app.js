...
const foodDetails = {
  ...
  '2026-09-24T15:15:00+08:00':{
    ...
    intro:'딤섬 점심과 베이징덕 저녁 사이에 상하이 게살면을 맛보는 곳입니다. 독립된 한 끼가 아니라 두 사람이 1그릇만 공유하는 것이 일정의 핵심입니다.', booking:'현재 예약하지 않음. 15:15 전후 현장 대기를 확인하고, 19:00 성용싱 방문 일정에 늦을 정도로 길면 생략하세요.',
    menu:[['蟹粉拌面','게살 비빔면','1그릇 공유'],['蟹粉小笼','게살 샤오롱바오','배가 남을 때만'],['姜茶','생강차','게 요리 곁들임']],
    order:'면과 게살 소스가 따로 나오면 먼저 면 일부에 소스를 섞어 농도를 조절하세요. 19시 성용싱 방문을 계획했으므로 추가 주문은 최소화합니다.',
  ...
  '2026-09-24T19:00:00+08:00':{
    ...
    intro:'와이탄에서 베이징덕과 야경을 함께 즐기는 여행의 메인 저녁입니다. 점심과 간식이 많으므로 예약 때 오리 반 마리 가능 여부를 확인해야 합니다.', booking:'추가 예약 1순위. 9월 24일(목) 19:00, 2명으로 자리 요청. 베이징덕 반 마리 주문 가능 여부, 오리 사전 준비 필요 여부, 창가석 가능 여부를 함께 문의하세요. 예약 완료 전에는 확정으로 표시하지 않습니다.',
...
const dayRoutes = [
  ...
  {title:'2일차 전체 동선',note:'호텔 → 다후춘 → 와이탄위안·예약 식당 → 와이탄·난징동루',stops:[['大壶春','다후춘 아침','大壶春 四川中路店','도보 또는 짧은 DiDi'],['外滩源','와이탄위안 아침 산책','外滩源','도보 · 11:40 도착 권장'],['御宝轩','Imperial Treasure','御宝轩 益丰外滩源店','도보'],['外滩','와이탄 강변','外滩','도보 또는 짧은 DiDi'],['庄氏隆兴面馆','장씨네 게살국수','庄氏隆兴面馆 浙江中路店','도보'],['南京东路','난징동루','南京东路步行街','도보·휴식'],['晟永兴 外滩店','성용싱 와이탄점','晟永兴 外滩店','도보'],['外滩·外滩源','와이탄 야경·빛축제','外滩','DiDi → 호텔']]},
